...
export const LabelErrorProvider = (props: ILabelErrorProvider) => {
  ...
  return (
    <div className={cn(
      ...
      { label && <label className="hdep-label-error-provider__label text-sm text-slate-900">{ label }</label> }
      <div className="hdep-label-error-provider__field">
        { children }
      </div>
      { error && <span className="hdep-label-error-provider__error text-sm text-red-500">{ error }</span> }
    </div>
  )
};
