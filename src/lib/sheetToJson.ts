...
export const sheetToJson = (sheet: ISheet) => {
  ...
  const columns = TABLE_COLUMNS.map((column) => {
    const [rowKey] =
      Object.entries(sheet).find(
        ([_, value]) => value.w &&
          prepareStringTitle(value.w as string) === (column.title as string).toLowerCase()
      ) || [];
  ...
  const dataSource = new Array(dataLength).fill(null).map((_, index) => {
    return columns.reduce(
      (acc, column) => {
        const key =
          column?.title === "Період підготовки"
        ...
        if (column?.title === "Період підготовки") {
          ...
          return {
            ...acc,
            [column.dataIndex]: {
              from: dayjs(new Date(get(sheet, key) as unknown as string)).format("YYYY-MM-DD"),
              to: dayjs(new Date(get(sheet, toKey) as unknown as string)).format("YYYY-MM-DD")
            }
          };
        }
